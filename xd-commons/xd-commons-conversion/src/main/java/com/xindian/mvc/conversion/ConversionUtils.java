...
import com.xindian.mvc.conversion.Conversion.DefaultDelimiter;
import com.xindian.mvc.conversion.Conversion.DefaultLocale;
import com.xindian.mvc.conversion.Conversion.DefaultTimeZone;
import com.xindian.mvc.conversion.Conversion.OnlyFirstToString;
		...
		if (fixedContext == null)
		{
			...
			if (member instanceof Field)
			{
				...
				if (fld.isAnnotationPresent(Trim.class))
				{
					...
					fixedContext.put(Converter.CONTEXT_STRING_TRIM_KEY, fld.getAnnotation(Trim.class).value());
				}
				if (fld.isAnnotationPresent(DefaultDelimiter.class))
				{
					logger.debug("  member DefaultDelimiter[" + fld.getAnnotation(DefaultDelimiter.class).value() + "]");
					fixedContext.put(Converter.CONTEXT_DELIMITER_KEY, fld.getAnnotation(DefaultDelimiter.class).value());
				}
				if (fld.isAnnotationPresent(DefaultDateFormats.class))
				{
					logger.debug("  member DefaultDateFormats[" + fld.getAnnotation(DefaultDateFormats.class).value() + "]");
					fixedContext.put(Converter.CONTEXT_DATE_TIME_PATTERN_ARRAY_KEY, fld.getAnnotation(DefaultDateFormats.class)
							.value());
				}
				if (fld.isAnnotationPresent(OnlyFirstToString.class))
				{
					logger.debug("  member OnlyFirstToString[" + fld.getAnnotation(OnlyFirstToString.class).value() + "]");
					fixedContext.put(Converter.CONTEXT_ONLY_FIRST_TO_STRING_FLAG_KEY, fld.getAnnotation(OnlyFirstToString.class)
							.value());
				}
				...
				{
					TimeZone timeZone = TimeZone.getTimeZone(fld.getAnnotation(DefaultTimeZone.class).value());
