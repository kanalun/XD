...
public class CollectionConverter extends AbstractConverter
{
	...
	public Object convert(Map<String, Object> context, Class targetType, Object sourceValue) throws ConversionException
	{
		...
		try
		{
			...
			if (co instanceof Collection<?>)//
			{
				...
				{
					String[] stringArray = (String[]) string2StringArray(context, (String) sourceValue);// String
																										// ->
																										// String[]
					...
					{
						Class<?> targetElementType = (Class<?>) context.get(CONTEXT_ELEMENT_TYPE_KEY);
						if (targetElementType == null)
						{
							targetElementType = String.class;
						}
						Object arrayTypeArray = Array.newInstance(targetElementType, 0);
						Object array = array2OtherTypeArray(context, arrayTypeArray.getClass(), stringArray);// String[]->
																												// Type[]
						int l = Array.getLength(array);
						for (int i = 0; i < l; i++)
						{
							// TODO USE Collections.addAll();?
							// put
							// array
							// to
							// Collection
				...
				} else if (sourceValue.getClass().isArray())// array ->
															// collection
				{
					Class<?> targetElementType = (Class<?>) context.get(CONTEXT_ELEMENT_TYPE_KEY);
					...
					Object arrayTypeArray = Array.newInstance(targetElementType, 0);// 这个数组并不做容器使用相当于new
																					// T[0],只作为参数让程序知道数组元素的类型
					Object array = array2OtherTypeArray(context, arrayTypeArray.getClass(), sourceValue);
					int l = Array.getLength(array);
					for (int i = 0; i < l; i++)
					{
						((Collection) co).add(Array.get(array, i));
				...
				{
					Class<?> elementType = (Class<?>) context.get(CONTEXT_ELEMENT_TYPE_KEY);
					if (elementType != null)
					{
						for (Iterator<?> sv = ((Collection<?>) sourceValue).iterator(); sv.hasNext();)
						{
							try
							{
								// convert element to targetElementType
								// TODO 优化一下:相同元素使用相同的类型转换器
								Object o = ConverterFactory.convert(context, elementType, sv.next());
								((Collection) co).add(o);
	...
	public static void main(String args[])
	{
		...
		logger.debug("==========" + collectionConverter.convert(context, Vector.class, new int[] { 55, 2, 3 }));
		...
		logger.debug("==========" + collectionConverter.convert(context, Vector.class, arrayList));

		logger.debug("=========="
				+ ((Vector) collectionConverter.convert(context, Vector.class, new String[] { "a", "b", "c" })).get(0).getClass());
		logger.debug("=========="
				+ ((Vector) collectionConverter.convert(context, Vector.class, new BigDecimal[] { new BigDecimal("1") })).get(0)
						.getClass());

		logger.debug("==========ccc" + ((Vector) collectionConverter.convert(context, Vector.class, arrayList)).get(0).getClass());

		logger.debug("==========" + ((Vector) collectionConverter.convert(context, Vector.class, "1,2,3")).get(0));
