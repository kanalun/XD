...
import com.xindian.mvc.conversion.ConversionUtils;
...
public class Bean5
{
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T> T buildObject(Class<T> type) throws InstantiationException, IllegalAccessException, ConversionException
	{
		T object = null;
		...
		try
		{
			while (resolver.hasNested(name))// 有嵌套,不能是基本类型!
			{
				...
				} else if (resolver.isIndexed(next))// 包含了两层:->p[index],1如果p不存在,创建p,
				{
					...
					{
						throw new UnnameException("Property[" + property + ":" + listField.getType() + "] In Bean["
			...
			} else if (resolver.isIndexed(name))// 包含了两层:->p[index],1如果p不存在,创建p,
			{
				...
				} else if (List.class.isAssignableFrom(listFiled.getType()))//
				{
					...
					set(nestedBean, index + "", ConverterFactory.convert(getContext(field), targetType, value));
				} else
				{
					throw new UnnameException("Property[" + property + ":" + listFiled.getType() + "] In Bean[" + bean.getClass()
