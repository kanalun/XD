 ...
 * {@code StrLookup} it is able to provide values for variables that appear in
 * expressions.
 * </p>
 * <p>
 * The basic idea of this class is that it can maintain a set of primitive
 * {@code StrLookup} objects, each of which is identified by a special prefix.
 * The variables to be processed have the form <code>${prefix:name}</code>.
 ...
 * possible to define a default lookup object, which will be used for variables
 * that do not have a prefix or that cannot be resolved by their associated
 * lookup object.
 * </p>
 * <p>
 ...
 * methods {@code registerGlobalLookup()} and {@code deregisterGlobalLookup()}.
 ...
 * <td>The {@code const} prefix indicates that a variable is to be interpreted
 * as a constant member field of a class (i.e. a field with the <b>static
 * final</b> modifiers). The name of the variable must be of the form
 ...
 * modified using the {@code registerLookup()} and {@code deregisterLookup()}
 * methods. The default lookup object (that is invoked for variables without a
 * prefix) can be set with the {@code setDefaultLookup()} method. (If a
 * {@code ConfigurationInterpolator} instance is created by a configuration
 * object, this lookup points to the configuration itself, so that variables are
 * resolved using the configuration's properties. This ensures backward
 * compatibility to earlier version of Commons Configuration.)
 * </p>
 * <p>
 ...
 * intended that each instance is associated with a single {@code Configuration}
 * object and used for its interpolation tasks.
 * </p>
 * 
 * @version $Id: ConfigurationInterpolator.java 1295276 2012-02-29 21:11:35Z
 *          oheger $
 * @since 1.4
 * @author <a
...
public class ConfigurationInterpolator extends StrLookup
{
	 ...
	 * 
	 * @param prefix
	 *            the variable prefix (must not be <b>null</b>)
	 * @param lookup
	 ...
	 * by the {@code lookup()} method. This implementation will check whether a
	 * lookup object is registered for the given prefix. If not, a <b>null</b>
	 * lookup object will be returned (never <b>null</b>).
	 * 
	 * @param prefix
	 *            the prefix
	 ...
	 * 
	 * @param interpolator
	 *            the instance receiving the local lookups
	 * @since upcoming
	 */
	...
	/**
	 * Sets the parent interpolator. This object is used if the interpolation is
	 * nested hierarchically and the current interpolation object cannot resolve
	 * a variable.
	 * 
	 * @param parentInterpolator
	 *            the parent interpolator object or <b>null</b>
	 * @since upcoming
	 */
	 ...
	 * interpolation is nested hierarchically and the current interpolation
	 * 
