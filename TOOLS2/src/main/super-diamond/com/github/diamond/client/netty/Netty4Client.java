...
public class Netty4Client
{
	...
	private static final ScheduledThreadPoolExecutor reconnectExecutorService = new ScheduledThreadPoolExecutor(2,
			new NamedThreadFactory("ClientReconnectTimer", true));
	...
	public Netty4Client(String host, int port, ClientChannelInitializer channelInitializer) throws Exception
	{
		...
		try
		{
			...
			logger.info("Start " + getClass().getSimpleName() + " " + NetUtils.getLocalAddress() + " connect to the server "
					+ host);
		} catch (Throwable t)
		{
			throw new Exception("Failed to start " + getClass().getSimpleName() + " " + NetUtils.getLocalAddress()
	...
	/*
	 * 使用时，循环调用该方法获取服务端返回的信息。 receiveMessage是阻塞方法，如果没有消息会等待。
	 */
	...
	private void doConnect() throws Throwable
	{
		...
		try
		{
			...
			{
				throw new Exception("client failed to connect to server " + getRemoteAddress() + ", error message is:"
						+ future.cause().getMessage(), future.cause());
			} else
			{
				throw new Exception("client failed to connect to server " + getRemoteAddress() + " client-side timeout "
	...
	private synchronized void initConnectStatusCheckCommand()
	{
		if (reconnectExecutorFuture == null || reconnectExecutorFuture.isCancelled())
		{
			...
			reconnectExecutorFuture = reconnectExecutorService.scheduleWithFixedDelay(connectStatusCheckCommand, 2 * 1000,
