...
public class ProxyDirContext implements DirContext
{
	...
	public ProxyDirContext(Hashtable<String, String> env, DirContext dirContext)
	{
		...
		if (dirContext instanceof BaseDirContext)
		{
			...
			if (baseDirContext.isCached())
			{
				try
				{
					cache = (ResourceCache) Class.forName(cacheClassName).newInstance();
				} catch (Exception e)
				{
					throw new IllegalArgumentException(sm.getString("resources.invalidCache", cacheClassName), e);
				}
	...
	{
		if (((!entry.exists) || (entry.context != null) || ((entry.resource != null) && (entry.resource.getContent() != null)))
	...
	protected void cacheLoad(CacheEntry entry)
	{
		...
		// Load object content
		if ((exists) && (entry.resource != null) && (entry.resource.getContent() == null)
