...
/**
 * A <b>Container</b> is an object that can execute requests received from a
 * client, and return responses based on those requests. A Container may
 * optionally support a pipeline of Valves that process the request in an order
 * configured at runtime, by implementing the <b>Pipeline</b> interface as well.
 * <p>
 ...
 * most likely containing one or more subcontainers that are either Host or
 * Context implementations, or other custom groups.
 * <li><b>Host</b> - Representation of a virtual host containing a number of
 * Contexts.
 ...
 * levels described above. For example, an administration application embedded
 * within a network device (such as a router) might only contain a single
 * Context and a few Wrappers, or even a single Wrapper if the application is
 * relatively small. Therefore, Container implementations need to be designed so
 * that they will operate correctly in the absence of parent Containers in a
 * given deployment.
 * <p>
 * A Container may also be associated with a number of support components that
 * provide functionality which might be shared (by attaching it to a parent
 * Container) or individually customized. The following support components are
 * currently recognized:
 * <ul>
 * <li><b>Loader</b> - Class loader to use for integrating new Java classes for
 * this Container into the JVM in which Catalina is running.
 * <li><b>Logger</b> - Implementation of the <code>log()</code> method
 * signatures of the <code>ServletContext</code> interface.
 * <li><b>Manager</b> - Manager for the pool of Sessions associated with this
 * Container.
...
public interface Container extends Lifecycle
{
	...
	/**
	 * Set the parent class loader for this component. For {@link Context}s this
	 * call is meaningful only <strong>before</strong> a Loader has been
	 * configured, and the specified value (if non-null) should be passed as an
	 * argument to the class loader constructor.
	 * 
	 * @param parent
	 ...
	 * 
	 * @param request
	 *            Request (associated with the response) to log
	 * @param response
	 *            Response (associated with the request) to log
	 * @param time
	 *            Time taken to process the request/response in milliseconds
	 *            (use 0 if not known)
	 * @param useDefault
	 *            Flag that indicates that the request/response should be logged
	 *            in the engine's default access log
	 */
