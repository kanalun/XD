...
public class Bean3
{
	...
	protected static <T> T setObjectToArray(T array, int index, Object value)
	{
		...
		if (arrayType.isArray())
		{
			...
			} else
			{
				...
				array = (T) Array.newInstance(elementType, index + 1);// 创建一个指定组件类型和长度的新数组
			...
			NoSuchFieldException, InstantiationException
	{
		...
		while (resolver.hasNested(name))// 有嵌套,不能是基本类型!
		{
			...
			} else if (resolver.isIndexed(next))// 包含了两层:->p[index],1如果p不存在,创建p,
			{
				...
				{
					throw new MVCException("Property[" + property + ":" + listField.getType() + "] In Bean[" + bean.getClass()
	...
	{
		ParameterizedType pt = (ParameterizedType) Bean3.class.getField("list").getGenericType();
			...
			SecurityException, NoSuchFieldException, InstantiationException
	{
		...
		//
		Class<?> t = BeanUtilsTest.getField(Bean3.class, "us").getType();
		int d = 0;
		while (t.isArray())
		{
			d++;
			...
			SecurityException, NoSuchFieldException, InstantiationException
	{
		...
		setObjectToArray(users, 2, new User());
