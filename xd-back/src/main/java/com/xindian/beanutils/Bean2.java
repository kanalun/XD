...
public class Bean2
{
	...
	public static Object get(Object bean, String filedName) throws IllegalArgumentException, IllegalAccessException
	{
		...
		{
			return getObjectFromList((List) bean, Integer.parseInt(filedName));
		} else if (bean instanceof Object[])
		{
			return getObjectFromArray(bean, Integer.parseInt(filedName));
		} else
		{
	...
	public static void set(Object bean, String filedName, Object value) throws IllegalArgumentException, IllegalAccessException
	{
		...
		{
			setObjectToList((List) bean, Integer.parseInt(filedName), value);
		} else if (bean instanceof Set<?>)
		{
			((Set) bean).add(value);
		} else if (bean instanceof Object[])
		{
			setObjectToArray(bean, Integer.parseInt(filedName), value);
		} else
		{
			Field field = BeanUtilsTest.getFiled(bean, filedName);
			if (field != null)
			...
			NoSuchFieldException, InstantiationException
	{
		...
		while (resolver.hasNested(name))// 有嵌套,不能是基本类型!
		{
			...
			} else if (resolver.isIndexed(next))// 包含了两层:->p[index],1如果p不存在,创建p,
			{
				...
				} else if (f.getType().isAssignableFrom(List.class))
				{
					...
					nestedBean = get(bean, index + "");// get the value from the
														// list

					if (nestedBean == null)
					{
						nestedBean = buildObject(GenericsUtils.getFieldGenericType(field, 0));
		...
		if (resolver.isMapped(name))
		{
			...
			Class<?> targetType = GenericsUtils.getFieldGenericType(mapFiled, 1);

			set(map, key, ConvertUtils.convert(value, targetType));// set the
																	// value the
																	// map
		...
		} else if (resolver.isIndexed(name))// 包含了两层:->p[index],1如果p不存在,创建p,
		{
			...
			if (f.getType().isArray())
			{
				...
				nestedBean = setObjectToArray(nestedBean, index,
						ConvertUtils.convert(value, nestedBean.getClass().getComponentType()));
			...
			} else if (f.getType().isAssignableFrom(List.class))
			{
				...
				if (nestedBean == null)// if the Array does not exist
				{
					nestedBean = buildObject(field.getType());// create a List
					set(bean, property, nestedBean);// put the Array back to the
													// bean
				}
				System.out.println("SET:" + nestedBean.getClass());

				set(nestedBean, index + "", ConvertUtils.convert(value, GenericsUtils.getFieldGenericType(field, 0)));
			} else
			{
				throw new MVCException("Property[" + property + ":" + f.getType() + "] In Bean[" + bean.getClass()
						+ "] Is Not A List Nor An Array");
		...
		} else
		{
			...
			System.out.println("	NAME " + name + " TYPE " + field.getType() + "	Value " + value);
			set(bean, name, value);
		}
	}
	...
	{
		ParameterizedType pt = (ParameterizedType) Bean2.class.getField("list").getGenericType();
