...
public class GetterSetterMappingFilter implements MappingFilter
{
	...
	public boolean filter(Mapping mapping) throws ErrorCodeException
	{
		...
		{
			throw new ErrorCodeException(404, "U can not use Setter(which is start as 'set') as an Action method..");
		}
		if (mapping.getMethodName() != null
				&& (mapping.getMethodName().trim().startsWith("get") || mapping.getMethodName().trim().startsWith("is")))
		{
