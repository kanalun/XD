...
/**
 * Replacement class of the original XMLWriter.java (version: 1.77) since the
 * original is still using StringBuffer which is not fast.
 */
public class XMLWriter extends XMLFilterImpl implements LexicalHandler
{
	...
	/**
	 * <p>
	 * Writes the <code>{@link org.dom4j.Element}</code>, including its
	 * <code>{@link Attribute}</code>s, and its value, and all its content
	 * (child nodes) to the current Writer.
	 * </p>
	 * 
	 * @param element
	...
	protected void writeDocType(DocumentType docType) throws IOException
	{
		if (docType != null)
		{
			...
			// writeDocType( docType.getElementName(), docType.getPublicID(),
			// docType.getSystemID() );
			writePrintln();
		}
	}
	...
	protected void writeAttributes(Element element) throws IOException
	{
		...
		for (int i = 0, size = element.attributeCount(); i < size; i++)
		{
			...
			if (ns != null && ns != Namespace.NO_NAMESPACE && ns != Namespace.XML_NAMESPACE)
			{
				...
				{ // output a new namespace declaration
					writeNamespace(ns);
					namespaceStack.push(ns);
				}
			}
