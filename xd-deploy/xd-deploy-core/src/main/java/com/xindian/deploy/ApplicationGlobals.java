...
import com.xindian.deploy.loader.ApplicationClassLoader;
...
{
	private static final Log log = LogFactory.getLog(ApplicationGlobals.class);

	private static String CRM_CONFIG_FILENAME = "conf" + File.separator + "Applications.xml";
	...
	/**
	 * Returns the global Locale used by Jive. A locale specifies language and
	 * country codes, and is used for internationalization. The default locale
	 * is system dependant - Locale.getDefault().
	 * 
	...
	public static Locale getLocale()
	{
		if (locale == null)
		{
			if (xmlProperties != null)
			{
				...
				// If no locale info is specified, return the system default
				// Locale.
	...
	/**
	 * Sets the global locale used by Jive. A locale specifies language and
	 * country codes, and is used for formatting dates and numbers. The default
	 * locale is Locale.US.
	 * 
	 * @param newLocale
	...
	/**
	 * Returns the global TimeZone used by Jive. The default is the VM's default
	 * time zone.
	 * 
	...
	public static void setHomeDirectory(String pathname)
	{
		...
		{
			Log.error("Error - the user running this application can not read " + "and write to the specified home directory ("
					+ pathname + "). " + "Please grant the executing user read and write permissions.");
		} else
		{
			home = pathname;
		}
	 ...
	 * </pre>
	 * 
	 * If the specified property can't be found, the <tt>defaultValue</tt> will
	 * be returned.
	 * 
	 * @param name
	 *            the name of the property to return.
	...
	/**
	 * Returns an integer value local property. Local properties are stored in
	 * the file defined in <tt>JIVE_CONFIG_FILENAME</tt> that exists in the
	 * <tt>home</tt> directory. Properties are always specified as
	 * "foo.bar.prop", which would map to the following entry in the XML file:
	 * 
	 * <pre>
	 * &lt;foo&gt;
	 *     &lt;bar&gt;
	...
	/**
	 * Returns a boolean value local property. Local properties are stored in
	 * the file defined in <tt>JIVE_CONFIG_FILENAME</tt> that exists in the
	 * <tt>home</tt> directory. Properties are always specified as
	 * "foo.bar.prop", which would map to the following entry in the XML file:
	 * 
	 ...
	 * 
	 * If the specified property can't be found, the <tt>defaultValue</tt> will
	 * be returned. If the property is found, it will be parsed using
	 ...
	 * exists, a new one will be created. Local properties are stored in the
	...
	/**
	 * Return all immediate children property values of a parent local property
	 ...
	 * of <tt>X.Y</tt> are <tt>A</tt>, <tt>B</tt>, and <tt>C</tt> (the value of
	 * <tt>C.D</tt> would not be returned using this method).
	 * <p>
	 * 
	...
	public static String getProperty(String name)
	{
		if (properties == null)
		{
			...
			properties = CRMProperties.getInstance();
		}
	...
	public static void migrateProperty(String name)
	{
		...
		if (getXMLProperty(name) != null)
		{
			...
			} else if (!getProperty(name).equals(getXMLProperty(name)))
			{
				Log.warn("Property '"
						+ name
						+ "' as specified in openfire.xml differs from what is stored in the database.  Please make property changes in the database instead of openfire.xml.");
			}
		}
	}

	/**
	 * Allows the name of the local config file name to be changed. The default
	 * is "openfire.xml".
	 * 
	 * @param configName
	...
	{
		CRM_CONFIG_FILENAME = configName;
	}
	...
	{
		return CRM_CONFIG_FILENAME;
	}

	/**
	 * Returns true if in setup mode. A false value means that setup has been
	 * completed or that a connection to the database was possible to properies
	 * stored in the datbase can be retrieved now. The latter means that once
	 * the database settings during the setup was done a connection to the
	 * datbase should be available thus properties stored from a previous setup
	 * will be available.
	 * 
