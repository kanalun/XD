...
public class LoggingEvent implements java.io.Serializable
{
	 ...
	 *             Please do not access it directly. Use the
	 *             {@link #getLoggerName} method instead.
	 * 
	 * */
	 ...
	 * again. Note that its value is always false when serialized. See also the
	 * getMDC and getMDCCopy methods.
	 */
	 ...
	 * 
	 * @param logger
	 *            The logger generating this event.
	 * @param level
	 ...
	 *            The logger generating this event.
	 * @param timeStamp
	 *            the timestamp of this logging event
	 * @param level
	 *            The level of this event.
	 * @param message
	 *            The message of this event.
	 ...
	 *            Fully qualified class name of Logger implementation.
	 * @param logger
	 *            The logger generating this event.
	 * @param timeStamp
	 *            the timestamp of this logging event
	 ...
	 *            MDC properties
	 */
	public LoggingEvent(final String fqnOfCategoryClass, final Category logger, final long timeStamp, final Level level,
			final Object message, final String threadName, final ThrowableInformation throwable, final String ndc,
			final LocationInfo info, final java.util.Map properties)
	{
		super();
		this.fqnOfCategoryClass = fqnOfCategoryClass;
		this.logger = logger;
	 ...
	 * If there is a local MDC copy, possibly because we are in a logging server
	 * or running inside AsyncAppender, then we search for the key in MDC copy,
	 * if a value is found it is returned. Otherwise, if the search in MDC copy
	 * returns a null result, then the current thread's <code>MDC</code> is
	 * used.
	 * 
	 * <p>
	...
	/**
	 * Set value for MDC property. This adds the specified MDC property to the
	 * event. Access to the MDC is not synchronized, so this method should only
	 * be called when it is known that no other threads are accessing the MDC.
	 * 
	 * @since 1.2.15
	 * @param propName
	 * @param propValue
	 ...
	 * 
	 * Equivalent to getMDC(String) in log4j 1.2. Provided for compatibility
	 * with log4j 1.3.
	 * 
	 * @param key
	 *            property name
	 ...
	 * byproduct of calling getLocationInformation).
	 * 
	...
	/**
	 * This removes the specified MDC property from the event. Access to the MDC
	 * is not synchronized, so this method should only be called when it is
	 * known that no other threads are accessing the MDC.
	 * 
	 * @param propName
